...
Cytoscape Web \cite{Lopes15092010} (http://http://cytoscapeweb.cytoscape.org/) can make a more beautiful
...
http://mcube.nju.edu.cn/jwang/lab/soft/cepa/. \texttt{read.gct} and \texttt{read.cls}
...
> res = cepa.all.parallel(mat = eset, label = label, pc = PID.db$NCI,
        glevel = "tvalue_sq", plevel = "mean", ncores = 4)
\end{Sinput}
\end{Schunk}
